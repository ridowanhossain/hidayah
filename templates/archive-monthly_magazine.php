...
<section class="archive-hero">
    <div class="archive-hero-content">
        ...
        <p><?php _e( 'All issues of the Monthly Islamic Magazine of Darbar Sharif are available here. Read, download, and share.', 'hidayah' ); ?> </p>
    </div>
</section>
...
<section class="archive-section">
    <div class="container">
        ...
        <nav aria-label="breadcrumb" class="archive-breadcrumb">
            ...
            <span class="breadcrumb-current"><?php _e( 'Monthly Haquer Dawat', 'hidayah' ); ?></span>
        </nav>
                ...
                <div class="col-inner">
                    ...
                    <div class="archive-toolbar">
                        <div class="archive-search-bar">
                            <form role="search" id="monthlyHdSearchForm" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" style="display: flex; width: 100%; align-items: center;">
                                ...
                                <input class="archive-search-input" id="monthlyHdSearchInput" placeholder="<?php _e( 'Search issues...', 'hidayah' ); ?>" type="text" name="s" value="<?php echo get_search_query(); ?>" />
                                <input type="hidden" name="post_type" value="monthly_magazine" />
                            </form>
                        </div>
                        <div class="archive-toolbar-right">
                            <select class="archive-sort-select" id="monthlyHdSortSelect">
                                <option value="newest" <?php selected($_GET['order'] ?? 'DESC', 'DESC'); ?>><?php _e( 'Newest First', 'hidayah' ); ?></option>
                                <option value="oldest" <?php selected($_GET['order'] ?? '', 'ASC'); ?>><?php _e( 'Oldest First', 'hidayah' ); ?></option>
                            </select>
                    ...
                    <div class="archive-count-header">
                        <div class="archive-count-badge" id="monthlyHdCountBadge">
                            ...
                            <?php printf( __( 'Total %s Magazines', 'hidayah' ), hidayah_en_to_bn_number($mag_query->found_posts) ); ?>
                        </div>
                        <div class="archive-year-filter">
                            <select id="monthlyHdYearFilter">
                                <option value=""><?php _e( 'All Years', 'hidayah' ); ?></option>
                                <?php foreach ( get_terms( array( 'taxonomy' => 'issue_year' ) ) as $yr ) : ?>
                                    <option value="<?php echo esc_attr( $yr->term_id ); ?>"><?php echo $yr->name; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="archive-year-filter">
                            <select id="monthlyHdCategoryFilter">
                                <option value=""><?php _e( 'Special Issues', 'hidayah' ); ?></option>
                            ...
                            <?php if ( $mag_query->have_posts() ) : while ( $mag_query->have_posts() ) : $mag_query->the_post();
                                ...
                                <article class="mhd-card <?php echo $is_special ? 'mhd-special' : ''; ?>">
                                    <div class="mhd-cover">
                                        ...
                                        <?php endif; ?>
                                        <?php if ( $pages ) : ?>
                                            <span class="mhd-page-badge"><?php printf( __( '%s Pages', 'hidayah' ), hidayah_en_to_bn_number($pages) ); ?></span>
                                        <?php endif; ?>
                                        <?php if ( $is_special ) : ?>
                                            <span class="mhd-special-badge"><?php _e( 'Special Issue', 'hidayah' ); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="mhd-info">
                ...
                <div class="col-inner">
                    ...
                    <div class="sidebar-widget">
                        <h4 class="sidebar-widget-title">
                            ...
                            <?php _e( 'Latest Issue', 'hidayah' ); ?>
                        </h4>
                        <?php
                        $latest_mag = new WP_Query( array( 'post_type' => 'monthly_magazine', 'posts_per_page' => 1 ) );
                        while ( $latest_mag->have_posts() ) : $latest_mag->the_post();
                            ...
                            <div class="mhd-latest-card">
                                ...
                                <div class="mhd-latest-info">
                                    <h5><?php the_title(); ?></h5>
                                    <p><?php if ( $l_pages ) printf( __( '%s Pages', 'hidayah' ), hidayah_en_to_bn_number($l_pages) ); ?> • <?php echo hidayah_en_to_bn_number(get_the_date()); ?></p>
                                    <a class="btn btn-sm" href="<?php the_permalink(); ?>">
                                        <span class="material-symbols-outlined">menu_book</span>
                                        <?php _e( 'Read & Download', 'hidayah' ); ?>
                                    </a>
                                </div>
                            </div>
                    ...
                    <div class="sidebar-widget">
                        <h4 class="sidebar-widget-title">
                            ...
                            <?php _e( 'Haquer Dawat Info', 'hidayah' ); ?>
                        </h4>
                        <p class="fs-14 text-light lh-1-7">
                            <?php echo h_opt( 'magazine_intro', 'Monthly Haquer Dawat — An Islamic monthly magazine published by Darbar Sharif. This magazine enriched with Islamic knowledge, Tasawwuf, Fiqh and contemporary issues has been published since 2016.' ); ?>
                        </p>
                    </div>
                                ...
                                <li>
                                    <a href="<?php the_permalink(); ?>">
                                        ...
                                        <div class="recent-info">
                                            <h5><?php the_title(); ?></h5>
                                            <span><?php printf( __( 'Read %s times', 'hidayah' ), hidayah_en_to_bn_number(number_format_i18n( $v_count )) ); ?></span>
                                        </div>
                                    </a>
                                </li>
                            <?php endwhile; wp_reset_postdata(); ?>
