...
function hidayah_filter_monthly_magazine_callback() {
    ...
    if ( $mag_query->have_posts() ) :
        while ( $mag_query->have_posts() ) : $mag_query->the_post();
            hidayah_render_monthly_magazine_card();
        endwhile;
    else :
        echo '<div class="col-full no-results-found"><p class="no-results">' . __( 'Sorry, no issues were found.', 'hidayah' ) . '</p></div>';
    endif;
    ...
    echo '<div class="ajax-count-data" style="display:none;">' . $mag_query->found_posts . '</div>';
    ...
    wp_send_json_success( array(
        'html'  => $html,
        'count' => $mag_query->found_posts,
    ) );
}
add_action( 'wp_ajax_filter_monthly_magazine', 'hidayah_filter_monthly_magazine_callback' );
add_action( 'wp_ajax_nopriv_filter_monthly_magazine', 'hidayah_filter_monthly_magazine_callback' );

if ( ! function_exists( 'hidayah_render_monthly_magazine_card' ) ) {
    function hidayah_render_monthly_magazine_card() {
        ...
        <article class="mhd-card <?php echo $is_special ? 'mhd-special' : ''; ?>">
            <div class="mhd-cover">
                ...
                <?php endif; ?>
                <?php if ( $pages ) : ?>
                    <span class="mhd-page-badge"><?php printf( __( '%s pages', 'hidayah' ), $pages ); ?></span>
                <?php endif; ?>
                <?php if ( $is_special ) : ?>
